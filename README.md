...
local Tab = Window:NewTab("☠️SOLDIER☠️")
...
Section:NewButton("Free Admin", "It Will Give You Admin Power...", function()
    ...
    SaveChatlogs.MouseButton1Down:Connect(function()
        if writefileExploit() then
            if #scroll_2:GetChildren() > 0 then
                ...
                local writelogs = '-- GlobalWaveOfficial Chat logs for "'..placeName..'"\n'
                for _, child in pairs(scroll_2:GetChildren()) do
                    writelogs = writelogs..'\n'..child.Text
                end
    ...
    CMDs = {}
    CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the GlobalWaveOfficial support server.'}
    ...
    task.spawn(function()
        ...
        minimizeHolder()
        if table.find({Enum.Platform.IOS, Enum.Platform.Android}, UserInputService:GetPlatform()) then notify("Unstable Device", "On mobile, GlobalWaveOfficial may have issues or features that are not functioning correctly.") end
    end)
end)
-- stolen code from one of my other scripts lol
local Tab = Window:NewTab("☠️OPTION☠️")
local Section = Tab:NewSection("other stuff")
Section:NewButton("Save", "OMGGG!!!", function()
local person = game:GetService("Players").LocalPlayer.Name
wait(.1)
local args = {
    [1] = person
...
Section:NewKeybind("Open/Close GUI", "[x_x]", Enum.KeyCode.E, function()
	Library:ToggleUI()
end)
